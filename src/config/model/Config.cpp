...
void Config::setIndex(const std::string& index) { _index = index; }
...
void Config::initLocation(const std::string& path) {
	if (_location.find(path) != _location.end())
		throw ConfigException(
			"[emerg] Invalid configuration: duplicate location");
...
void Config::setLocationIndex(const std::string& path,
...
const std::string& Config::getLocationRoot(const std::string& path) const {
	std::map<std::string, ConfigLocation>::const_iterator it =
		_location.find(path);
	if (it == _location.end()) {
...
const std::vector<std::string>& Config::getLocationAllowMethods(
